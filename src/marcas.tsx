/**
 * Íconos de Material Symbols (Apache-2.0), inlineados como paths.
 *
 * Van como SVG y no como fuente de íconos a propósito: el póster se exporta a
 * PNG con html-to-image y una fuente externa con ligaduras se rompe ahí.
 * Pintan con `currentColor`, así heredan el color de la paleta y nunca chocan.
 */

export type Marca = { id: string; nombre: string; path: string };

/** Marcas para los puestos vendidos. Los `id` no cambian: van guardados en la config. */
export const MARCAS: Marca[] = [
  ...
  },
  {
    id: 'verificado',
    nombre: 'Verificado',
    path: 'M23 12l-2.44-2.79.34-3.69-3.61-.82-1.89-3.2L12 2.96 8.6 1.5 6.71 4.69 3.1 5.5l.34 3.7L1 12l2.44 2.79-.34 3.7 3.61.82 1.89 3.2 3.4-1.47 3.4 1.46 1.89-3.19 3.61-.82-.34-3.69L23 12zm-12.91 4.72l-3.8-3.81 1.48-1.48 2.32 2.33 5.85-5.87 1.48 1.48-7.33 7.35z',
  },
  {
    id: 'check',
    nombre: 'Confirmado',
  ...
  },
  {
    id: 'candado',
    nombre: 'Candado',
    path: 'M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zM9 6c0-1.66 1.34-3 3-3s3 1.34 3 3v2H9V6zm3 12c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2z',
  },
  {
    id: 'boleta',
    nombre: 'Boleta',
  ...
  },
  {
    id: 'rombo',
    nombre: 'Hexágono',
    path: 'M17.2 3H6.8l-5.2 9 5.2 9h10.4l5.2-9L17.2 3z',
  },
  {
    id: 'punto',
    nombre: 'Punto',
...
/** Íconos de la interfaz: cerrar, ganador, cuenta en espera. Mismo origen. */
export const SIMBOLOS = {
  ...
  reloj: 'M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm.5-13H11v6l5.25 3.15.75-1.23-4.5-2.67V7z',
  bloqueado:
    'M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2zm5 13.59L15.59 17 12 13.41 8.41 17 7 15.59 10.59 12 7 8.41 8.41 7 12 10.59 15.59 7 17 8.41 13.41 12 17 15.59z',
  // Mismos trazos que las marcas del tablero: un solo dibujo por concepto.
  corazon: marcaPorId('corazon').path,
  boleta: marcaPorId('boleta').path,
  candado: marcaPorId('candado').path,
  compartir:
...
export function Icono({ id, className }: { id: string; className?: string }) {
  return <Svg d={marcaPorId(id).path} className={className} />;
}

export function IconoUI({ id, className }: { id: Simbolo; className?: string }) {
  return <Svg d={SIMBOLOS[id]} className={`icono-ui${className ? ` ${className}` : ''}`} />;
}
